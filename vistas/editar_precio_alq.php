...
<html>
    ...
    <body>
         ...
         <form name="insertar" action="../vistas/editar_precio_alq.php?cod=<?php echo $_GET['cod'] ?>&fact=<?php echo $_GET['fact'] ?>&codigo" method="post" enctype="multipart/form-data">
                <table>
							<tr>
                                                           <td><label>Alquiler</label></td>
                                                           <td><label>Precio : </label></td>
                                                           <td><label>Anexos : </label></td>
                                                        
                                                                   <?php
                                                                   include "../modelo/conexion.php";
                                                           $consulta= "select a.*, b.* from equipos_asig a, alquiler b where a.cod_equipo=b.codigo and a.facturado='".$_GET['cod']."'";
                                                          ...
                                                          $c +=1;$p +=$fila['precio_a'];
                                                            ?>
                                                                   <tr>
                                                                      ...
                                                                      <input type="hidden" name="codigo<?php echo $c ?>" value="<?php echo $fila['id_equipo_a'] ?>">
                          ...
                          Pago:<input type="text" name="pago" value="" placeholder="Ingrese pago aqui">
                                                   
             <input type="hidden" name="cant" value="<?php echo $c ?>">
	...
	$anexo = $_POST["anexo$x"];
         $sql = "UPDATE `equipos_asig` SET  `precio_a`='".$precio."',`anexo`='".$anexo."' WHERE `id_equipo_a`='".$codigo."'";
        mysql_query($sql) or die(mysql_error());
                     $c += $precio;     
        }    
         if(isset($_GET["fact"])){
              $cambio = valorEnLetras($c); 

         $sql = "UPDATE `recibo_caja` SET  `copagos`='".$_POST['pago']."',`total`='".$c."',`letras`='".$cambio."' WHERE `numero_recibo`='".$_GET["fact"]."'";
        mysql_query($sql) or die(mysql_error());
        }
