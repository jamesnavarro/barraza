...
$requestp=mysql_query('SELECT b.id_paciente FROM equipos_asig a, ordenes b, pacientes c WHERE'
        . ' c.id_paciente=b.id_paciente and b.id=a.numero_orden_a and b.facturado like "'.$estado.'%"'
        . ' and b.fecha_registro like "'.$fecha.'%"'
        . ' and c.nombres like "%'.$nombre.'%" and c.apellidos like "%'.$apellido.'%" and c.numero_doc like "%'.$documento.'%"'
        . ' and c.id_empresa like "'.$empresa.'%" and c.regimen like "%'.$regimen.'%" '
        . ' and a.autorizacion like "'.$orden.'%" group by b.id_paciente ');
...
$request=mysql_query('SELECT *, max(b.fecha_registro) as a, max(b.fecha_final) as b, (a.facturado) as fa FROM equipos_asig a, ordenes b, pacientes c WHERE'
        . ' c.id_paciente=b.id_paciente and b.id=a.numero_orden_a and b.facturado like "'.$estado.'%"'
        . ' and b.fecha_registro like "'.$fecha.'%"'
        . ' and c.nombres like "%'.$nombre.'%" and c.apellidos like "%'.$apellido.'%" and c.numero_doc like "%'.$documento.'%"'
        . ' and c.id_empresa like "'.$empresa.'%" and c.regimen like "%'.$regimen.'%" '
        . ' and a.autorizacion like "'.$orden.'%" group by b.id_paciente order by a desc '.$limit);
 $cont = 0;
 $table='';
     ...
     $cont = $cont + 1 ;
         ...
         $c='<a href="../vistas/alquiler_proceso.php?eliminar='.$row["id_equipo_a"].'"><img src="../imagenes/eliminar.png" alt="ver" height="20px" width="20px"></a>';
             ...
             if($row["regimen"]=='4'){
                  ...
                  $check='';
                   ...
                   if($row["facturado"]!='Facturado'){
                       $check = '<input type="checkbox" name="valor'.$cont.'" value="'.$row["numero_orden_a"].'">';
           ...
           $table = $table.'<tr>'
                            ...
                            . '<td>'.$ver.''.$color.''.$row["nombres"].' '.$row["apellidos"].' '.$row["apellido2"].''.$row["id_empresa"].'<font></a></td>'
                            . '<td>'.$color.''.$r.'</td>'
                            ...
                            . '<td>'.$b.'</td>'
                            . '<td>'.$c.'</td>'
                            . '<td>'.$check.'</td><td>'.$check2.'</td></tr>';
}
$table = $table.'<tr><td colspan="11" style="text-align:right"><i>Total de Archivos: </i>'
        . ' <input type="text" name="cant"  style="width:20px;height:20px;"  value="'.$cont.'">'
        . '<select name="tipo" required>'
        . '<option value="FAC">Factura</option>'
        . '<option value="REC">Recibo</option>'
        . '</select>'
        . '<input type="button" name="buscar" value="Facturar" class="alt_btn" onclick="facturar()">';
