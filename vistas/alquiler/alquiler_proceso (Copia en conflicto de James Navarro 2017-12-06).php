...
$request=mysql_query('SELECT * FROM equipos_asig a, ordenes b, pacientes c WHERE c.id_paciente=b.id_paciente and b.id=a.numero_orden_a and b.estado_ord="En proceso" group by b.id ');
$c =0;
...
<body onload="<?php $das = $_SESSION['t'];if(!empty($das['options'])) echo $das['options']; ?>">
		...
		<article class="module width_full">
                                    ...
                                    <form name="buscarA" action="" method="post" enctype="multipart/form-data">
                                     <div>
                                        ...
                                        <table class="table table-bordered table-striped table-hover" id="">
                                            ...
                                            <td><label># Documento : </label><td><input name="documento" style="width:130px;height:20px;"></td>
                                                <td nowrap><label>Estado : </label><td><select name="estado" style="width:180px;">
                                            ...
                                            <td><label>Nombre : </label><td><input name="nombre" style="width:130px;height:20px;"></td>
                                                <td><label>Regimen :</label><td><select name="regimen" style="width:180px;">
                                            ...
                                            <td><label>Apellido : </label><td><input name="apellido" style="width:130px;height:20px;"></td>
                                                <td><label>Empresa : </label><td>
                                                    <select name="empresa" style="width:180px;">
                                                                   <option value="">..Seleccione..</option>
                                                                   <?php
                                                 ...
                                                 <tr>
                                                      <td><label>Autorizacion :</label><td><input name="orden1" style="width:130px;height:20px;"></td>
                                                <td><label>Mes:</label><td>
                                                <select name="ano" style="width:130px;">
                                            ...
                                            <tr>
                                                ...
                                                <td><label>Estado de facturacion : </label><td><select name="factura" style="width:180px;">
                                                                  <option value="">Seleccione Estado</option>
            ...
            <?php
if($page>1){?>
	<a href="../vistas/?id=all_alquiler&page=1"><img src="../images/a1.png"></a>
	<a href="../vistas/?id=all_alquiler&page=<?php echo $page - 1;?>"><img src="../images/a11.png"></a>
<?php
}else{
...
<?php
if($page<$last_page){?>
	<a href="../vistas/?id=all_alquiler&page=<?php echo $page + 1;?>"><img src="../images/p1.png"></a>
	<a href="../vistas/?id=all_alquiler&page=<?php echo $last_page;?>"><img src="../images/p11.png"></a>
<?php
}else{
...
$request=mysql_query('SELECT a.*, c.*, b.* FROM equipos_asig a, ordenes b, pacientes c WHERE c.id_paciente=b.id_paciente and b.id=a.numero_orden_a and b.estado_ord="En proceso" group by b.id  '.$limit);
}

if($nom !='' || $ape !=''|| $reg !=''|| $doc !=''|| $orden !=''|| $estado !=''|| $mes !=''|| $emp !=''|| $fact !=''){
    $request=mysql_query("SELECT a.*, b.*, c.* FROM equipos_asig a, ordenes b, pacientes c WHERE b.facturado LIKE '".$fact."%' and c.id_empresa LIKE '".$emp."%' and a.fecha_a LIKE '%".$mes."%' and b.estado_ord LIKE '%".$estado."%' and a.autorizacion LIKE '%".$orden."%' and c.nombres LIKE '%".$nom."%' and c.apellidos LIKE '%".$ape."%' and c.numero_doc LIKE '%".$doc."%' and c.regimen LIKE '%".$reg."%'  and b.id=a.numero_orden_a and  c.id_paciente=b.id_paciente group by b.id ");
}}
else{
...
if($request){
    ?>
         <form name="buscarA" action="../vistas/?id=all_alquiler&codi=facturar" method="post" enctype="multipart/form-data">
        <?php
...
$table = '<table class="table table-bordered table-striped table-hover" id="">';
             ...
             $table = $table.'<thead >';
              ...
              $table = $table.'<th>'.'Orden Interna (Alq).'.'</th>';
...
$table = $table.'</thead>';
 ...
 while($row=mysql_fetch_array($request))
	{    
     $cont = $cont + 1 ;
     $look ='<td><a href="../vistas/?id=add_detalle_alquiler&cod='.$row["numero_orden_a"].'&codigo_pac='.$row["id_paciente"].'"><img src="../imagenes/ojo.png" alt="ver" height="20px" width="20px"></a></td>';
         $ver='<a href="../vistas/?id=add_detalle_alquiler&cod='.$row["numero_orden_a"].'&codigo_pac='.$row["id_paciente"].'">';
         $b='<a href="../form_editar/formulario_editar_alquiler.php?codigo='.$row["id_equipo_a"].'"><img src="../imagenes/modificar.png" alt="ver" height="20px" width="20px"></a>';
          $c='<a href="../vistas/?id=add_detalle_alquiler&eliminar='.$row["numero_orden_a"].'"><img src="../imagenes/eliminar.png" alt="ver" height="20px" width="20px"></a>';
         ...
         mysql_query($sqlr);
           ...
           if($row["regimen"]=='1'){$r= "Contributivo";}
            ...
            $mes = $row["fecha_a"];
             $t = substr("$mes",0, -3);
             if($t == date("2013-01")){$m = 'Enero';}
             if($t == date("2013-02")){$m = 'Febrero';}
             if($t == date("2013-03")){$m = 'Marzo';}
             if($t == date("2013-04")){$m = 'Abril';}
             if($t == date("2013-05")){$m = 'Mayo';}
             if($t == date("2013-06")){$m = 'Junio';}
             if($t == date("2013-07")){$m = 'Julio';}
             if($t == date("2013-08")){$m = 'Agosto';}
             if($t == date("2013-09")){$m = 'Septiembre';}
             if($t == date("2013-10")){$m = 'Octubre';}
             if($t == date("2013-11")){$m = 'Noviembre';}
             if($t == date("2013-12")){$m = 'Diciembre';}
             if($t == date("2014-01")){$m = 'Enero';}
             if($t == date("2014-02")){$m = 'Febrero';}
             if($t == date("2014-03")){$m = 'Marzo';}
             if($t == date("2014-04")){$m = 'Abril';}
             if($t == date("2014-05")){$m = 'Mayo';}
             if($t == date("2014-06")){$m = 'Junio';}
             if($t == date("2014-07")){$m = 'Julio';}
             if($t == date("2014-08")){$m = 'Agosto';}
             if($t == date("2014-09")){$m = 'Septiembre';}
             if($t == date("2014-10")){$m = 'Octubre';}
             if($t == date("2014-11")){$m = 'Noviembre';}
             if($t == date("2014-12")){$m = 'Diciembre';}
             if($t == date("2015-01")){$m = 'Enero';}
             if($t == date("2015-02")){$m = 'Febrero';}
             if($t == date("2015-03")){$m = 'Marzo';}
             if($t == date("2015-04")){$m = 'Abril';}
             if($t == date("2015-05")){$m = 'Mayo';}
             if($t == date("2015-06")){$m = 'Junio';}
             if($t == date("2015-07")){$m = 'Julio';}
             if($t == date("2015-08")){$m = 'Agosto';}
             if($t == date("2015-09")){$m = 'Septiembre';}
             if($t == date("2015-10")){$m = 'Octubre';}
             if($t == date("2015-11")){$m = 'Noviembre';}
             if($t == date("2015-12")){$m = 'Diciembre';}
             if($t == date("2016-01")){$m = 'Enero';}
             if($t == date("2016-02")){$m = 'Febrero';}
             if($t == date("2016-03")){$m = 'Marzo';}
             if($t == date("2016-04")){$m = 'Abril';}
             if($t == date("2016-05")){$m = 'Mayo';}
             if($t == date("2016-06")){$m = 'Junio';}
             if($t == date("2016-07")){$m = 'Julio';}
             if($t == date("2016-08")){$m = 'Agosto';}
             if($t == date("2016-09")){$m = 'Septiembre';}
             if($t == date("2016-10")){$m = 'Octubre';}
             if($t == date("2016-11")){$m = 'Noviembre';}
             if($t == date("2016-12")){$m = 'Diciembre';}
             if($row["regimen"]=='4'){
                   if($row["facturado"]!='Facturado'){$check2 = '<input type="checkbox" name="valorp'.$cont.'" value="'.$row["numero_orden_a"].'">';}else{$check2='';}
                  $check='';
             }else{
                   if($row["facturado"]!='Facturado'){$check = '<input type="checkbox" name="valor'.$cont.'" value="'.$row["numero_orden_a"].'">';}else{$check='';}
                    $check2='';
             }

           if($row["autorizacion"]=='Pendiente'){$aut = '<font color="purple">Pendiente';}else{$aut = $row["autorizacion"];}
           $table = $table.'<tr><td>'.$color.''.$row["numero_orden_a"].'<font></a></td><td>'.$row["oi"].'<font></a></td><td>'.$ver.''.$aut.'</font></td><td>'.$ver.''.$led.''.$color.''.$row["nombres"].' '.$row["apellidos"].'<font></a></td><td>'.$color.''.$r.'</font></td>
...
$sql1 = "SELECT MAX(numero_factura) as id_inc FROM facturas";
...
$sql11 = "SELECT sum(precio_a) as total FROM equipos_asig WHERE numero_orden_a='".$_POST["valor$x"]."'";
...
$cambio = valorEnLetras($total); 
    ...
    //se guarda la factura
        ...
        $alquiler= 'alquiler';
   	$sql = "INSERT INTO `facturas`(`copagos`,`cod_alquiler`, `fechai`, `fechaf`,`letras`,`numero_factura`, `id_paciente`, `orden_int`, `orden_ext`, `forma_pago`, `meses`, `pago_pendiente`, `fecha_ven`, `total`, `informacion`, `fecha_registro`)";
