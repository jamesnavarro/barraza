<?php 
$request=mysql_query('select count(*) from actividades where tarea="Reunion"');
if($request){
                      ...
                      <!-- START Widget Collapse -->
                        ...
                        <div class="span12 widget dark stacked">
                            ...
                            <header>
                                ...
                                <h4 class="title">Lista de Reuniones</h4>
...
$request=mysql_query("SELECT * FROM actividades where tarea='Reunion' ");
  }
	...
	{       
           $sql1 = "SELECT * FROM sis_contacto where id_contacto=".$row['id_contacto']."";
            ...
            $table = $table.'<tr><td width="5%"><a href="../vistas/?id=add&cod='.$row['Id'].'"><img src="../imagenes/cerrar.png"></a></td>
                <td width="15%"><a href="../vistas/?id=ver_reunion&cod='.$row['Id'].'">'.$row['Subject'].'</font></td>
                <td width="15%">'.$ver.'</td>
                    <td width="10%">'.$row["relacionado"].'</td>
               <td class="hidden-phone">'.$row["StartTime"].'</font></td><td class="hidden-phone">'.$row["user"].'</font></td>
                            <td class="hidden-phone"><a href="../vistas/?id=reunion&cod='.$row["Id"].'"><img src="../imagenes/modificar.png"></a></td>
                                <td class="hidden-phone"><a href="../vistas/?id=reuniones&del='.$row["Id"].'"><img src="../imagenes/eliminar.png"></a></td></tr>';   
...
<?php
 if(isset($_GET['del'])){
     ...
     echo '<script lanquage="javascript">alert("Usted no tiene permiso para eliminar");location.href="../vistas/?id=reuniones"</script>'; 
}else{
$sql = "DELETE FROM actividades WHERE Id=".$_GET['del']."  and user='".$_SESSION['k_username']."'";
mysql_query($sql, $conexion);
echo "<script language='javascript' type='text/javascript'>";
echo "location.href='../vistas/?id=reuniones'";
echo "</script>";
}
      }
?>