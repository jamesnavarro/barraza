...
if($page8>1){?>
	<a href="../vistas/id=ordenes&page8=1"><img src="../images/a1.png"></a>
	<a href="../vistas/?id=ordenes&page8=<?php echo $page8 - 1;?>"><img src="../images/a11.png"></a>
<?php
}else{
	?><img src="../images/ant.png"><?php
}
?>
(Pagina <?php echo $page8;?> de <?php echo $last_page;?>)
<?php
if($page8<$last_page){?>
	<a href="../vistas/?id=ordenes&page8=<?php echo $page8 + 1;?>"><img src="../images/p1.png"></a>
	<a href="../vistas/?id=ordenes&page8=<?php echo $last_page;?>"><img src="../images/p11.png"></a>
<?php
}else{
...
$s = $fila1["idcaso"];
?>
<a href="../vistas/solicitudes.php" title="ver solicitudes de atenciones, de click aqui para ver">Solicitudes de ordenes internas(<font color="red"><?php echo $s ?></font>)</a>
<?php
//Esta es la cadena limit que anexaremos a nuestra consulta
$limit = 'LIMIT ' .($page8 - 1) * $rows_by_page .',' .$rows_by_page;
...
$request=mysql_query('SELECT a.*, b.*, c.* FROM actividad a, pacientes b, ordenes c where a.id_paciente=b.id_paciente and c.id_paciente=b.id_paciente and a.archivo=c.id  group by orden_servicio desc  '.$limit);
}
if($oi !='' ||$nom !='' || $ape !=''|| $doc !=''|| $ord !='' || $est !='' || $fac !='' || $desde !='' || $arc !='' || $factura !='' || $empresa!=''){
...
$request=mysql_query("SELECT a.*, b.*, c.* FROM actividad a, pacientes b, ordenes c where ".$f." ".$linea." a.prioridad like '%".$factura."%' and b.id_empresa like '".$empresa."%'  and a.orden_servicio like '".$oi."%' and a.Location like '%".$fac."%' and b.nombres like '%".$nom."%' and b.apellidos like '%".$ape."%' and b.numero_doc like '%".$doc."%' and a.orden_externa like '%".$ord."%' and a.archivo=c.id and a.id_paciente=b.id_paciente and a.user LIKE '%".$arc."%' group by a.orden_servicio");
    ...
    if($_SESSION["area"]=='OFICINA'){
    $request=  mysql_query('SELECT a.*, b.*, c.* FROM actividad a, pacientes b, ordenes c where a.prioridad="Facturado" and a.aviso!="En proceso" and a.estado="Completada" and a.id_paciente=b.id_paciente and c.id_paciente=b.id_paciente and a.archivo=c.id group by orden_servicio desc '.$limit);

}else{
    $request=  mysql_query('SELECT a.*, b.*, c.* FROM actividad a, pacientes b, ordenes c where a.user="'.$_SESSION['k_username'].'" and a.prioridad="Facturado" and a.aviso!="En proceso" and a.estado="Completada" and a.id_paciente=b.id_paciente and c.id_paciente=b.id_paciente and a.archivo=c.id group by orden_servicio  desc ');

}}
if($request){
    //    echo'<hr>';
    $table = '<table class="table table-bordered table-striped table-hover" id="tabla8">';
             $table = $table.'<thead >';
...
$tt=0;
	   ...
	   {       $tt = $tt + 1;
           ...
           $req2=mysql_query('select count(*) from reportes where id_orden='.$row["orden_servicio"].' ');
                                        $re = mysql_fetch_row($req2);
                                        if($re[0]!=0){
	                                $caso =  '<i>Incidencias: '.$num_inc = $re[0].'</i> <img src="../imagenes/ledrojo.gif">';
                                        }else{ $caso = '';}
                                        
           $table = $table.'<tr><td width="5%">'.$led.' '.$ver2.''.$row["orden_servicio"].'<font></a></td><td width="5%">'.$ver.''.$row["orden_externa"].'<font></a></td><td class="hidden-phone">'.$row["Description"].'<br>'.$caso.'</a></td><td width="5%">'.$row["cant"].'<font></a></td>'.$por.'</td><td class="hidden-phone">'.$ver2.''.$et.'<font></a></td><td class="hidden-phone">'.$row["Location"].'<font></a></td><td class="hidden-phone">'.$sa.'<font></a></td><td class="hidden-phone">'.$row["numero_doc"].'<font></a></td>
