...
if(isset($_GET['no-iniciada'])){
  if($_SESSION['area']=='OFICINA'){
   $request=mysql_query('SELECT a.*, b.*, c.* FROM actividad a, pacientes b, ordenes c where a.id_contacto="" and a.prioridad!="Facturado" and a.id_paciente=b.id_paciente and c.id_paciente=b.id_paciente and a.archivo=c.id group by orden_servicio desc ');  
    }else{
         $request=mysql_query('SELECT a.*, b.*, c.* FROM actividad a, pacientes b, ordenes c where a.user="'.$_SESSION['k_username'].'" and  a.id_contacto="" and a.prioridad!="Facturado" and a.id_paciente=b.id_paciente and c.id_paciente=b.id_paciente and a.archivo=c.id group by orden_servicio asc ');  
...
if(isset($_GET['completadas'])){
   if($_SESSION['area']=='OFICINA'){
    $request=mysql_query('SELECT a.*, b.*, c.* FROM actividad a, pacientes b, ordenes c where a.id_contacto>98 and a.Location="" and a.prioridad!="Facturado" and a.id_paciente=b.id_paciente and c.id_paciente=b.id_paciente and a.archivo=c.id group by orden_servicio desc ');
           ...
           $request=mysql_query('SELECT a.*, b.*, c.* FROM actividad a, pacientes b, ordenes c where a.user="'.$_SESSION['k_username'].'" and  a.id_contacto>98 and a.Location="" and a.prioridad!="Facturado" and a.id_paciente=b.id_paciente and c.id_paciente=b.id_paciente and a.archivo=c.id group by orden_servicio desc ');
...
$request=mysql_query('SELECT a.*, b.*, c.* FROM actividad a, pacientes b, ordenes c where a.prioridad!="Facturado" and a.id_paciente=b.id_paciente and c.id_paciente=b.id_paciente and a.archivo=c.id group by orden_servicio  '.$limit);
}
...
if(isset($_GET['no-iniciada'])){
  if($_SESSION['area']=='OFICINA'){
    $request=mysql_query('SELECT a.*, b.*, c.* FROM actividad a, pacientes b, ordenes c where a.id_contacto="" and a.prioridad!="Facturado" and a.id_paciente=b.id_paciente and c.id_paciente=b.id_paciente and a.archivo=c.id group by orden_servicio desc '.$limit);
    }else{
         $request=mysql_query('SELECT a.*, b.*, c.* FROM actividad a, pacientes b, ordenes c where a.user="'.$_SESSION['k_username'].'" and a.id_contacto="" and a.prioridad!="Facturado" and a.id_paciente=b.id_paciente and c.id_paciente=b.id_paciente and a.archivo=c.id group by orden_servicio  order by a.fecha_reg_ta asc '.$limit);
...
if(isset($_GET['en-proceso'])){
    ...
    $request=mysql_query('SELECT a.*, b.*, c.* FROM actividad a, pacientes b, ordenes c where a.id_contacto>0 and a.id_contacto<98 and a.prioridad!="Facturado" and a.id_paciente=b.id_paciente and c.id_paciente=b.id_paciente and a.archivo=c.id group by orden_servicio desc '.$limit);
          ...
          $request=mysql_query('SELECT a.*, b.*, c.* FROM actividad a, pacientes b, ordenes c where a.user="'.$_SESSION['k_username'].'" and  a.id_contacto>0 and a.id_contacto<98 and a.prioridad!="Facturado" and a.id_paciente=b.id_paciente and c.id_paciente=b.id_paciente and a.archivo=c.id group by orden_servicio  order by a.fecha_reg_ta asc '.$limit);
...
if(isset($_GET['completadas'])){
  if($_SESSION['area']=='OFICINA'){
    $request=mysql_query('SELECT a.*, b.*, c.* FROM actividad a, pacientes b, ordenes c where a.id_contacto>98 and a.Location="" and a.prioridad!="Facturado" and a.id_paciente=b.id_paciente and c.id_paciente=b.id_paciente and a.archivo=c.id group by orden_servicio desc '.$limit);
           ...
           $request=mysql_query('SELECT a.*, b.*, c.* FROM actividad a, pacientes b, ordenes c where a.user="'.$_SESSION['k_username'].'" and  a.id_contacto>98 and a.Location="" and a.prioridad!="Facturado" and a.id_paciente=b.id_paciente and c.id_paciente=b.id_paciente and a.archivo=c.id group by orden_servicio  order by a.fecha_reg_ta desc '.$limit);
...
if($_SESSION['area']=='OFICINA'){
    ...
    $request=mysql_query('SELECT a.*, b.*, c.* FROM actividad a, pacientes b, ordenes c where a.Location!="" and a.prioridad="Facturado" and a.id_paciente=b.id_paciente and c.id_paciente=b.id_paciente and a.archivo=c.id group by orden_servicio desc '.$limit);
           ...
           $request=mysql_query('SELECT a.*, b.*, c.* FROM actividad a, pacientes b, ordenes c where a.user="'.$_SESSION['k_username'].'" and  a.Location!="" and a.prioridad="Facturado" and a.id_paciente=b.id_paciente and c.id_paciente=b.id_paciente and a.archivo=c.id group by orden_servicio order by a.fecha_reg_ta desc  '.$limit);
...
$tt=0;
	...
	{    
           ...
           $table = $table.'<tr><td width="5%">'.$led.' '.$ver2.''.$row["orden_servicio"].' '.$e.'<font></a></td><td width="5%">'.$ver2.''.$row["orden_externa"].'<font></a>'
