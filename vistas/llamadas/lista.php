<?php 
$request=mysql_query('select count(*) from actividades where tarea="Llamada"');
if($request){
                      ...
                      <!-- START Widget Collapse -->
                        ...
                        <div class="span12 widget dark stacked">
                            ...
                            <header>
                                ...
                                <h4 class="title">Lista de Llamadas</h4>
...
$request=mysql_query("SELECT * FROM actividades where tarea='Llamada' ");
  }
...
//    echo'<hr>';
       $table = '<table class="table table-bordered table-striped table-hover" id="tabla">';
             ...
             $table = $table.'<thead >';
              ...
              $table = $table.'<th width="2%">'.'Cerrado'.'</th>'; 
              $table = $table.'<th width="2%">'.'Direccion'.'</th>';
	...
	{       
            ...
            $table = $table.'<tr><td width="2%"><a href="../vistas/?id=add&cod='.$row['Id'].'&add=Completada"><img src="../imagenes/cerrar.gif"></a></td>
                <td width="2%">'.$row['prioridad'].'</td><td width="15%"><a href="../vistas/?id=ver_llamada&cod='.$row['Id'].'">'.$row['Subject'].'</a></td>
               <td width="15%">'.$ver.'</td>
                    <td width="10%">'.$row["relacionado"].'</td>
               <td class="hidden-phone">'.$row["StartTime"].'</font></td><td class="hidden-phone">'.$row["user"].'</font></td>
                            <td class="hidden-phone"><a href="../vistas/?id=llamada&cod='.$row["Id"].'"><img src="../imagenes/modificar.png"></a></td>
                                <td class="hidden-phone"><a href="../vistas/?id=llamadas&del='.$row["Id"].'"><img src="../imagenes/eliminar.png"></a></td></tr>';   
...
<?php
 ...
 if(isset($_GET['del'])){
     ...
     echo '<script lanquage="javascript">alert("Usted no tiene permiso para eliminar");location.href="../vistas/?id=llamadas"</script>'; 
}else{
$sql = "DELETE FROM actividades WHERE Id=".$_GET['del']." and user='".$_SESSION['k_username']."'";
mysql_query($sql, $conexion);
echo "<script language='javascript' type='text/javascript'>";
echo "location.href='../vistas/?id=llamadas'";
echo "</script>";
}
}
?>