...
$(function() {
     // 2- Insertar registro
		...
		if(zon.length>0 && naci.length>0 && doc.length>0 && est.length>0 && nom.length>0 && ape.length>0 && ap2.length>0 && ced.length>0 && tel.length>0 && fam.length>0 && tfa.length>0 && dir.length>0 && emp.length>0 && tipo.length>0){
			$.ajax({
				type: 'GET',
     ...
     // 4- Buscar en la tabla
		...
		var comprobar = $('#fotoo').val().length;
		...
		if(comprobar>0){		
			...
			var datos = formulario.serialize();		
			var archivos = new FormData();			
			var url = '../vistas/clientes/subir_ordenes.php';		
			for (var i = 0; i < (formulario.find('input[type=file]').length); i++) { 			
...
function consentimiento(id){
    ...
    window.open('../vistas/pacientes/informado.php?id='+id+'','Form','width=900,height=650');
