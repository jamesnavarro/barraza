...
include('../../modelo/conexion.php');
                    ...
                    if($est==""){
                        ...
                        if($est==99){
                            ...
                            $linea='a.id_contacto>0 and a.id_contacto<=97 and';$fac='activa';
                        
                        }else{
                            $linea='a.id_contacto>1 and a.id_contacto<=98 and';$fac='activa';
                        }
...
if(isset($_GET['page'])){
            ...
            $request=mysql_query("SELECT * FROM actividad a, pacientes b, ordenes c where a.id_paciente=b.id_paciente and c.id_paciente=b.id_paciente and a.archivo=c.id and a.user LIKE '%".$use."%' $colx group by orden_servicio desc");
            if($request){
...
<!--                            <table class="table table-bordered table-striped table-hover" id="">-->
                                    ...
                                    $table = '';
                                        ...
                                        if($est==""){
                                            ...
                                            if($est==99){
                                                ...
                                                $linea='a.id_contacto>1 and a.id_contacto<=97 and';$fac='activa';   
                                            }
                                            }
                                        if($desde=='' && $hasta==''){$f='';}else{$f ='a.fecha_reg_ta>="'.$desde.'" and a.fecha_reg_ta<="'.$hasta.'" and ';}
                                         if($rev=='x'){$revidas=' and a.Location="" ';}else if($rev=='Revisado'){$revidas=' and a.Location="Revisado" ';}else{$revidas='';}
          ...
          $sql = mysql_query("SELECT * FROM actividad a, pacientes b, ordenes c where a.id_paciente=b.id_paciente and c.id_paciente=b.id_paciente and a.archivo=c.id  and a.user LIKE '".$use."%' $col group by orden_servicio desc ".$limit);
			$item = 0;
...
$tt=0;
	...
	{    
           ...
           $table = $table.'<tr id="'.$row["orden_servicio"].'"><td width="5%">'.$led.' '.$ver2.''.$row["orden_servicio"].' '.$e.'<font></a><br>'.$btn.'</td>'
                   ...
                   . '<td class="hidden-phone">'.$row["Description"].'<br>'.$caso.'</a></td>'
                   . '<td width="5%">'.$row["cant"].'<font></a></td>'.$por.'</td>'
                   . '<td class="hidden-phone">'.$ver2.''.$et.'<font></a><br>'.$se.'</td>'
