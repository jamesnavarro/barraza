...
if(isset($_GET['codigo'])){
 $consulta= "select a.*, b.* from actividad a, pacientes b where a.orden_servicio='".$_GET['codigo']."' and a.id_paciente=b.id_paciente GROUP BY orden_externa";
$result=  mysql_query($consulta);
while($fila=  mysql_fetch_array($result)){
$name=$fila['nombres'].' '.$fila['apellidos'];  
 $id=$fila['id_paciente'];
}
    $consult= "select * from motivo_consulta WHERE  id_orden=".$_GET["codigo"]."";
...
<body onload="doScroll()" onunload="window.name=document.body.scrollTop">
    ...
    <form  action="<?php if(isset($_GET['editar'])){ echo '../modelo/editar_consulta.php?edit='.$id_orden.''; }else{echo '../modelo/insertar_consulta.php?paciente='.$id.'';} ?>" method="post" enctype="multipart/form-data">
		...
		<article class="module width_full">
                    ...
                    <div class="module_content"> 
                     ...
                     <table>
                         ...
                         <tr> 
                             <td><textarea style="width:100%;" rows="8" name="Motivo"><?php if(isset($motivo)){echo $motivo; } ?></textarea> </td>
                                 <td></td>
                                 </tr>
                     </table>
                        </fieldset>
			...
			<header><h3>EXAMEN FÌSICO</h3></header>
                                    ...
                                    <table>
                                        ...
                                        <tr>
                                          ...
                                          <td><textarea style="width:60%;" rows="8" name="fisico"><?php if(isset($fisico)){echo $fisico;} ?></textarea> </td>
                                      </tr>
                                    </table>
