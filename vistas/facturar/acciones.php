...
switch ($_GET['sw']) {
        case 0:
               $sqlr = "UPDATE `actividad` SET Description='".$_GET['de']."',cod_aten='".$_GET['cod']."',orden_externa='".$_GET['oe']."',precio_total='".$_GET['pr']."',cuota_pagada='".$_GET['cop']."'  WHERE `orden_servicio`='".$_GET["id"]."';";
              $ver = mysql_query($sqlr) or die(mysql_error());
              echo 'Se actualizo con exito. '.$ver;
            break;
         case 1:
        ...
        case 3:
                  ...
                  $sql = "INSERT INTO `facturas`(`tipo`,`copagos`,`letras`,`fechai`, `fechaf`,`numero_factura`, `id_empresa`, `forma_pago`, `meses`, `pago_pendiente`, `total`, `fecha_registro`, `regimen`)";
                  $sql.= "VALUES ('FAC','".$copagos."','".$cambio."','".$fi."', '".$fv."','".$factura."', '".$emp."', '".$forma_pago."', '".$meses."', '".$pago_pendiente."', '".$t."', '".$fecha_reg."', '".$regimen."')";
	          mysql_query($sql);
         ...
         if ($_GET['id']=='true') {
             $a = '1';
              ...
              $a = '0';
               ...
               $sqlr = "UPDATE actividad SET checki='$a'  WHERE orden_servicio='".$_GET["orden"]."';";
               $ver = mysql_query($sqlr) or die(mysql_error());
               echo 'Se actualizo con exito. '.$_GET['id'].$a;
            break;
             case 5:
        
               $sqlr = "UPDATE actividad SET Location='Revisado'  WHERE orden_servicio='".$_GET["orden"]."';";
               $ver = mysql_query($sqlr) or die(mysql_error());
               echo 'Se actualizo con exito. '.$_GET['id'].$a;
            break;
        case 6:
        
               $sqlr = "UPDATE actividad SET orden_externa='".$_GET["aut"]."', precio_total='".$_GET["a"]."'  WHERE orden_servicio='".$_GET["id"]."';";
               $ver = mysql_query($sqlr) or die(mysql_error());
               echo 'Se actualizo con exito. '.$_GET['id'].$a;
            break;
        case 7:
        
               $sqlr = "UPDATE actividad SET precio_total='".$_GET["aut"]."'  WHERE orden_servicio='".$_GET["id"]."';";
               $ver = mysql_query($sqlr) or die(mysql_error());
               echo 'Se actualizo con exito. '.$_GET['id'].$a;
            break;
