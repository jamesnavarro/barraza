...
<input  type="text" id="pre" value="" placeholder="Valor Atencion"> <button  id="factu" class="btn btn-primary" onclick="autorizaciones2();">Cambiar Precio</button>
                         ...
                         if(isset($_GET['ano'])){
                             ...
                             $fec = ' and a.StartTime like "'.$fecha.'%" ';
                         }else{
                             $fec = '';
                         }
                         echo $_GET['empresa'];
			...
			if(mysql_num_rows($sql)>0){
				...
				while($mostrar = mysql_fetch_array($sql)){
					$co = $co+1;
                                        ...
                                        $total=($mostrar['precio_total']*$mostrar['cant']);
                                        $pago += $total;
...
//                                            }
                                        ...
                                        <td><input type="text" id="oe'.$mostrar['orden_servicio'].'" value="'.$mostrar['orden_externa'].'"  onchange="up_orden('.$mostrar['orden_servicio'].')" style="width:50px"><br>'.$n.' '.$et.'</td>'
                                        . '<td>'.$mostrar['documento'].':'.$mostrar['numero_doc'].'-'.substr($mostrar['Subject'],13).'<br> '.$regimen.'<br> '.$mostrar['tipo_s'].'</td>
                                            <td><input type="text" id="co'.$mostrar['orden_servicio'].'" value="'.$mostrar['cod_aten'].'" style="width:30px" onchange="up_codigo('.$mostrar['orden_servicio'].')">                                        
                                            <td><textarea rows="3" cols="15" id="de'.$mostrar['orden_servicio'].'" disabled>'.$mostrar['Description'].'</textarea></td>'
                                        . '<td style="text-align:right;"><input type="text" id="pr'.$mostrar['orden_servicio'].'" onchange="up_orden('.$mostrar['orden_servicio'].')" value="'.$mostrar['precio_total'].'"  style="width:50px">'
                                        . ''
                                                . '<input type="hidden" id="precio'.$mostrar['orden_servicio'].'" onchange="up_orden('.$mostrar['orden_servicio'].')" value="'.$mostrar['precio_total'].'" style="width:30px"></td>'
                                        . '<td style="text-align:right;"><input type="text" id="copago'.$mostrar['orden_servicio'].'" disabled onchange="up_orden('.$mostrar['orden_servicio'].')" value="'.$mostrar['cant'].'" style="width:30px"></td>'
                                                . '<td style="text-align:right;"><input type="text" id="cop'.$mostrar['orden_servicio'].'" onchange="up_orden('.$mostrar['orden_servicio'].')" value="'.$mostrar['cuota_pagada'].'"  style="width:40px">'
                                        . '<td style="text-align:right;"><input type="text" id="precio_total'.$mostrar['orden_servicio'].'" value="'.($total-$mostrar['cuota_pagada']).'" style="width:40px" disabled></td>
