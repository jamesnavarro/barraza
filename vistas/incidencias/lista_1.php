<?php 
$request=mysql_query('select count(*) from sis_incidencias where estado_inc="Solucionado" ');
if($request){
...
$dates = $_POST['ano'].'-'.$_POST['mes'];
    $request=mysql_query("SELECT * FROM sis_incidencias a, pacientes b where a.id_paciente=b.id_paciente and a.estado_inc='Solucionado' and concat(asunto_inc,' ',registrado_por,' ',nombres,' ',apellidos) like '%".$_POST['asunto']."%' and a.categoria_inc like '%".$_POST['categoria']."%' and a.fecha_registro_inc like '%".$dates."%'");
}else{
$request=mysql_query("SELECT * FROM sis_incidencias a, pacientes b where a.id_paciente=b.id_paciente and a.estado_inc='Solucionado' ");
  }
