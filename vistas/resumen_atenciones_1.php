...
$request=Connection::runQuery('select * from actividad where estado="Completada" and archivo='.$_GET['orden'].' group by orden_servicio');
     ...
     $request=Connection::runQuery('select * from actividad where user="'.$_SESSION['k_username'].'" and estado="Completada" and archivo='.$_GET['orden'].' group by orden_servicio');   
}}
if(isset($_GET['orden_externa'])) {  
    if($_SESSION["admin"] == 'Si'){
$request=Connection::runQuery('select * from actividad where estado="Completada" and orden_externa="'.$_GET['orden_externa'].'" group by orden_servicio');
    }else{
     $request=Connection::runQuery('select * from actividad where user="'.$_SESSION['k_username'].'" and estado="Completada" and orden_externa='.$_GET['orden_externa'].' group by orden_servicio');   
}}
if($request){
//    echo'<hr>';
