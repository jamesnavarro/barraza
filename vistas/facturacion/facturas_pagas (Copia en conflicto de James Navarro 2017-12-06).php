...
$request=mysql_query('select count(*) from facturas where facturas_anulada!=0 ');
...
$request=mysql_query('select * from facturas a, pacientes b where a.factura_anulada!=0 and a.id_paciente=b.id_paciente '.$fa.' '.$limit);
...
$table = $table.'</thead>';
	...
	{     
            if($row["cod_alquiler"]==''){$ver2='<a href="../vistas/?id=facturacion_anulada&fact='.$row["factura_anulada"].'">';}else{$ver2='<a href="../vistas/?id=facturacion_2&fact='.$row["numero_factura"].'">';}
           if($ver_prod=='Habilitado'){$ver='<a href="../vistas/?id=ver_paciente&cod='.$row["id_paciente"].'">';}else{$ver='';}
            ...
            if($row["cod_alquiler"]==''){
                ...
                $c='<a target="_blank" href="../imprimir_alquiler.php?imprimir='.$row["numero_factura"].'"><img src="../imagenes/imp.png" alt="ver" height="20px" width="20px"></a>';
            }
