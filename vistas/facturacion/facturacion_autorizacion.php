...
$consulta2= "select a.*, b.*, c.*, max(a.fecha_mod_ta), min(a.fecha_mod_ta) from actividad a, pacientes b, sis_empresa c where a.factura='".$_GET['est']."' and a.relacionado='".$_GET['factura']."' and a.id_paciente=b.id_paciente and b.id_empresa=c.rips group by a.orden_externa";
$result2=  mysql_query($consulta2);
while($fila=  mysql_fetch_array($result2)){
$idpac=$fila['id_paciente'];
$archivo=$fila['archivo'];
...
if(isset($_GET['factura'])){
    if($_GET['t']=='FAC'){
$consulta2= mysql_query("select * from facturas where numero_factura=".$_GET['factura']." and estado='".$_GET['est']."' ");
...
ventana_secundaria.close() 
                ...
                <article class="module width_full">
                    <header><h3 align="center">EDITAR-FACTURA</h3></header>
			<header><h3>Servicios prestados/elementos</h3>
...
$request6=mysql_query('select a.*, b.* from productos_vendidos a, productos b where a.cod_pro=b.codigo_interno and a.autorizacion="'.$_GET['fact'].'"');
      ...
      if(isset($_GET['factura'])){              
$request=mysql_query('select *, max(cant_ins) as c, count(cant_ins) as t from actividad where estado="Completada" and relacionado="'.$_GET['factura'].'"  and factura="'.$_GET['est'].'" group by orden_servicio');
...
//    echo'<hr>';
       $table = '<table class="table table-bordered table-striped table-hover" id="">';
             ...
             $table = $table.'<thead >';
              ...
              $table = $table.'<th>Vlr Und</th>';
              $table = $table.'<th>Copagos</th>';
...
$table = $table.'</thead>';
	...
	{       
            $c1 +=1;
                $st1=($row['precio_total']*$row['t']);
                $subt=($st1-$row['cuota_pagada']);
                $t1=$t1+$st1;
                $cuota += $row['cuota_pagada'];
                if(isset($_GET['factura'])){$f='&factura='.$_GET['factura'].'';}else{$f='';}
               $table = $table.'<tr><td>'.$row["orden_externa"].'</td><td><input type="text" name="ate'.$c1.'" value="'.$row["cod_aten"].'"  style="width:100px"></td>'
                       . '<td>'.$row['Description'].'</td><td><input type="text" name="anexo'.$c1.'" value="'.$row['anexo'].'" style="width:100px"></td><td>'.$row['t'].
                        '</td><td><input type="text" name="precio'.$c1.'" value="'.$row['precio_total'].'" style="width:60px">'
                       . '<td><input type="text" name="preciocop'.$c1.'" value="'.$row['cuota_pagada'].'" style="width:60px">'
                       . '<input type="hidden" name="id'.$c1.'" value="'.$row['Id'].'">'
                       . '<input type="hidden" name="tipo'.$c1.'" value="atencion"></td><td>'.$subt.'</td>
                            </tr>';
             
	}
        $table = $table.'<tr><td colspan="5"></td><td>'.$t1.'</td><td>'.$cuota.'</td><td>'.($t1-$cuota).'</td>';
        $t2=0;
	...
	{       
		...
		$table = $table.'<tr><td>'.$row["autorizacion"].'</td><td>'.$row["cod_insumo"].'</td>'
                        . '<td>'.$row['nombre_insumo'].'</td>'
                        . '<td><input type="text" name="anexo'.$c2.'" value="'.$row["inf_adicional"].'"></td><td>'.$row['cant_usada'].
                        '</td><td><input type="text" name="precio'.$c2.'" value="'.$row['sub_precio'].'"></td>'
	...
	{     $c3 +=1;
               ...
               $st4=$row['sub_precio_m']*$row['cantidad_usada'];
                ...
                if(isset($_GET['factura'])){$f='&factura='.$_GET['factura'].'';}else{$f='';}
                          $table = $table.' ';
		$table = $table.'<tr><td>'.$row["autorizacion"].'</td>'
                        . '<td>'.$row["cod_med"].'</td>'
                        . '<td>'.$row['nombre_medicamento'].'</td>'
                        . '<td><input type="text" name="anexo'.$c3.'" value="'.$row["info"].'"></td><td>'.$row['cantidad_usada'].
                        '</td><td><input type="text" name="precio'.$c3.'" value="'.$row['sub_precio_m'].'"></td><td>'.$row['sub_precio_m']*$row['cantidad_usada'].''
...
<tr>
    ...
    <td>$ <input type="text" readonly name="total" value="<?php echo $total; ?>"></td>
                ...
                <form name="insertar_empresa" action="<?php echo '../modelo/c_f.php?t='.$_GET['t']; ?>" method="post" enctype="multipart/form-data">
		<article class="module width_full">
			<header><h3>Facturacion</h3></header>
				...
				<div class="module_content"> 
                           <br>
                                        ...
                                        include "../modelo/conexion.php";
$sql1 = "SELECT MAX(numero_factura) as ida FROM facturas where estado='' ";
...
<tr>
    ...
    <td>$ <input type="text" name="copagos" value="<?php echo $cuota; ?>"></td>
...
<tr>
    ...
    <td>$ <input type="text" readonly name="saldo" value="<?php echo $total-$cuota; ?>"></td>
