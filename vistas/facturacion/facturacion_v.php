...
if(isset($_GET['fact'])){
$consulta2= "select * from equipos_ventas where numero_orden_a='".$arch."'";
...
if(isset($_GET['fact'])){
$consulta2= "select a.*, b.*, c.* from equipos_ventas a, pacientes b, sis_empresa c, ordenes d where d.id='".$arch."' and d.id=a.numero_orden_a and d.id_paciente=b.id_paciente and b.id_empresa=c.rips group by a.id_equipo_a";
$result2=  mysql_query($consulta2);
while($fila=  mysql_fetch_array($result2)){
...
//////////
 ...
 ?>
                ...
                <form name="insertar_empresa"  class="span12 widget shadowed dark form-horizontal bordered"  action="../modelo/editar_pago.php?editar=<?php echo $_GET['fact']; ?>" method="post" enctype="multipart/form-data">
                    ...
                    <header><h4 class="title">Factura # : <?php echo $num_fact ?></h3>
                                <?php 
    if($editar_fac=='Habilitado'){ 
        ?>
                            <a href="../vistas/?id=facturacion_ventas&fact=<?php echo $orden_int ?>&factura=<?php echo $_GET['fact'].'&estado='.$estado ?>"><img src="../imagenes/estrella.png">
    <?php }     ?>
                                </h4>
				...
				<div class="module_content"> 
                           <br>
                                        <hr>
                                            <a href="../vistas/?id=facturas"> <input type="button" name="enviar" value="Cancelar" class="alt_btn"></a><?php ; ?>
                                          <a target="_blank" href="../imprimir_ventas.php?imprimir=<?php echo $num_fact.'&estado='.$estado ?>"><input type="button" name="bo" value="Imprimir"/> </a>
    ...
    if(isset($_GET['fact'])){  ?>
                ...
                <header><h4 class="titulo">PRODUCTOS VENDIDOS</h4></header>
                        <hr>
...
$request2=mysql_query('select a.*, b.* from equipos_ventas a, productos b where a.cod_equipo=b.codigo and a.numero_orden_a="'.$arch.'"');
...
$request3=mysql_query('select a.*, b.* from equipos_asig a, alquiler b where a.cod_equipo=b.codigo and a.facturado="'.$_GET['fact'].'"');
        $t3=$t2;
        while($row=mysql_fetch_array($request3))
	{       
                $st2=$row['precio_a']*$row['cantidad']*$row['meses'];
                $t3=$t3+$st2;
		$table = $table.'<tr><td>'.$row["cod_equipo"].'</td><td>'.$row["autorizacion"].'</td><td>'.$row['nombre'].'</td><td></td><td>'.$row['cantidad'].
                        '</td>
                             <td>'.$row['meses'].'</td>
                                <td>'.$row['fecha_a'].'</td>
                                    <td>'.$row['fecha_f'].'</td><td>'.number_format($row['precio_a']).'</td><td>'.number_format($row['precio_a']*$row['cantidad']*$row['meses']).'</td></tr>';
