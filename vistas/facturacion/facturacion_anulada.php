...
if(isset($_GET['copiar'])){
    ...
    $query = mysql_query("select * from actividad where factura_anulada=".$_GET['fact']." group by orden_servicio ");
...
if(isset($_GET['fact'])){
$consulta2= "select a.*, b.*, c.* from actividad a, pacientes b, sis_empresa c where a.factura_anulada='".$_GET['fact']."' and a.id_paciente=b.id_paciente and b.id_empresa=c.rips group by a.factura_anulada";
...
<form name="insertar_empresa"  class="span12 widget shadowed dark form-horizontal bordered"  action="../modelo/editar_pago_1.php?editar=<?php echo $_GET['fact']; ?>" method="post" enctype="multipart/form-data">
    <header>
        <h4 class="title">Factura Anulada # : <?php echo $_GET['fact'] ?></h4>
        <?php 
...
<!--    <a target="_blank" href="../php-mysql.php?imprimir=<?php echo $_GET['fact'] ?>">
...
<?php 
     if(isset($_GET['fact'])){              
$request=mysql_query('select *, max(cant_ins) as c, count(cant_ins) as t  from actividad where estado="Completada" and factura_anulada='.$_GET['fact'].'  group by orden_servicio');

$request3=mysql_query('select a.*, b.* from insumos_asignados a, insumos b where  a.cant_usada!=0 and a.cod_insumo=b.codigo and a.factura_anulada="'.$_GET['fact'].'"');
$request4=mysql_query('select a.*, b.* from medicamentos_asig a, medicamentos b where a.cod_med=b.codigo_int and a.factura_anulada="'.$_GET['fact'].'"');
$request5=mysql_query('select a.*, b.* from laboratorio_asig a, laboratorio b where a.cod_lab=b.cod_lab and a.factura_anulada="'.$_GET['fact'].'"');
$request6=mysql_query('select a.*, b.* from productos_vendidos a, productos b where a.cod_pro=b.codigo_interno and a.factura_anulada="'.$_GET['fact'].'"');
if($request){
//    echo'<hr>';
