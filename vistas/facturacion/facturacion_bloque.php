...
if(isset($_GET['fact'])){
$consulta2= "select a.*, b.*, c.* from actividad a, pacientes b, sis_empresa c where a.relacionado='".$_GET['fact']."' and a.factura='".$_GET['est']."' and a.id_paciente=b.id_paciente and b.id_empresa=c.rips group by a.relacionado";
...
//////////
if(isset($_GET['fact'])){
$consulta3= "select * from facturas where numero_factura='".$_GET['fact']."' and estado='".$_GET['est']."'";
...
<form name="insertar_empresa"  class="span12 widget shadowed dark form-horizontal bordered"  action="../modelo/editar_pago_1.php?editar=<?php echo $_GET['fact']; ?>" method="post" enctype="multipart/form-data">
    ...
    if($editar_fac=='Habilitado'){ 
        ?>
        <a href="../vistas/?id=facturacion_autorizacion&factura=<?php echo $_GET['fact'] ?>&t=FAC&est=<?php echo $_GET['est'] ?>">  <img src="../imagenes/estrella.png"></a>
    ...
    <a target="_blank" href="../FacturaBloque.php?imprimir=<?php echo $_GET['fact'].'&est='.$_GET['est'] ?>">
    <input type="button" name="bo" value="Imprimir Resumido"/> 
    </a>   
    <a target="_blank" href="../FacturaBloque_1.php?imprimir=<?php echo $_GET['fact'].'&est='.$_GET['est'] ?>">
    <input type="button" name="bo" value="Imprimir Detalle"/> 
    </a> 
    ...
    <div class="module_content"> 
        ...
        <table class="table table-bordered table-striped table-hover" id=""> 
            <tr>
                <td><label>Factura por bloque:</label></td>
                <td></td>
                <td></td>
                <td></td>
                <td></td>
...
<?php 
     if(isset($_GET['fact'])){              
$request=mysql_query('select *, max(cant_ins) as c, count(cant_ins) as t  from actividad where factura="'.$_GET['est'].'" and estado="Completada" and relacionado='.$_GET['fact'].'  group by orden_servicio order by Subject asc');
...
$table = $table.'</thead>';
	...
	{      
            ...
            //$led = '<img src="../imagenes/ledrojo.gif">';
                ...
                $t1=$t1+$st1;
		$table = $table.'<tr><td>'.$row["orden_servicio"].$led.'</td>'
                        . '<td>'.$row["orden_externa"].'</td>'
                        . '<td>'.substr($row["Subject"],12).'</td>'
                        . '<td>'.$row["cod_aten"].'</td>'
                        . '<td>'.$row['Description'].'</td>'
                        . '<td>'.$row['anexo'].'</td><td>'.$row['t'].
                        '</td><td>'.$row['precio_total'].'</td><td>'.$row['cuota_pagada'].'</td><td>'.number_format(($row['precio_total']*$row['t'])-$row['cuota_pagada']).'</td></tr>';
