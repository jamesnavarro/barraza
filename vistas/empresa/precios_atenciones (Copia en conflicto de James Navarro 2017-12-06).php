...
<?php 
$request=mysql_query("SELECT a.*, b.nombre_atencion, b.codigo_atencion FROM precios_atenciones a, atenciones b WHERE a.id_empresa='".$_GET['cod']."' and a.id_atencion=b.id_atencion");
if($request){
//    echo'<hr>';
	...
	{       $co +=1;
           ...
           if($eliminar_prod=='Habilitado'){$c='<a href="../vistas/?id=precios_atenciones&cod='.$_GET['cod'].'&eliminar='.$row["id_precio"].'"><img src="../imagenes/eliminar.png" alt="ver" height="20px" width="20px"></a>';}else{$c='';}
            ...
            $table = $table.'<tr><td>'.$row["id_precio"].'<input type="hidden" name="id'.$co.'" value="'.$row["id_precio"].'" class="span6"></td></td>
               <td>'.$row["codigo_atencion"].'</font></td><td>'.$row["nombre_atencion"].'</font></td><td><input type="text" name="pago'.$co.'" value="'.$row["pagos"].'" class="span6"></td><td>$<input type="text" name="precio'.$co.'" value="'.$row["precio"].'" class="span6"></td>
 ...
 <?php
        ...
        if(isset($_POST['update']))
    {
        ...
        for($i=1;$i<=$C;$i++){
                ...
                mysql_query("update precios_atenciones set pagos='".$_POST['pago'.$i]."',precio='".$_POST['precio'.$i]."' where id_precio='".$_POST['id'.$i]."'  ");
         }
