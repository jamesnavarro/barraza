...
//	$this->Cell(0,10,'Pagina '.$this->PageNo().'/1',0,0,'C');
	...
	$fila = mysql_fetch_array($pacientess);
        ...
        $strConsulta2 = "select a.*, b.*, c.* from actividad a, pacientes b, sis_empresa c where a.relacionado='".$_GET['imprimir']."' and a.id_paciente=b.id_paciente and b.id_empresa=c.rips group by a.relacionado";
	...
	for ($i=0; $i<$numfilas; $i++)
		{
			...
			$pdf->SetFont('Arial','I',0);
    			...
    			$pdf->SetTextColor(0);
				$pdf->Row(array($fila['orden_servicio'], $fila['orden_externa'], $fila['cod_aten'], $fila['Description'], $fila['anexo'], $fila['count(a.cant_ins)'], number_format($fila['precio_total']), number_format($fila['count(a.cant_ins)*a.precio_total'])));
...
$pdf->Cell(0,3,'_______________________________________________________________________________________________________________________________________________',0,1,'C');
        ...
        $pdf->SetFont('Arial','',8);
        $pdf->Cell(0,3,'Pagos Realizados :  '.number_format($fila3['copagos']).'                  Servicios : '.number_format($at).'                Materiales y Medicamentos :'.number_format($ts),0,1);
        $pdf->Ln(0);
        $pdf->Cell(0,3,'Son :  '.$fila3['letras'],0,1);
        $pdf->Ln(0);
        $pdf->SetFont('Arial','B',12);
        $pdf->Cell(0,3,'Total Neto a Pagar $ '.number_format($fila3['total']),0,1,'R');
         $pdf->Cell(0,5,'Saldo Pendiente $ '.number_format($fila3['total']-$fila3['copagos']),0,1,'R');
        $pdf->Ln(8);
