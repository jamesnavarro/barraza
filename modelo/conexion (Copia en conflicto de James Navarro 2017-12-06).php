...
$dns = "mysql:host=localhost;dbname=mediso5_idb";
$user = "mediso5_idb";
$pass = "20031123003";
    ...
    $id = $objData->ida;
    $name = $objData->namea;
    $price = $objData->pricea;
    $fecha = $objData->fechaa;
    
    $pa = $objData->paa;
    $pulso = $objData->pulsoa;
    $fr = $objData->fra;
    $us = $objData->user;
    ...
    if($db){
        ...
        $result = $db->prepare("SELECT * FROM actividad where Id='".$id."'");
        $result->execute();
        $fi = $result->fetch();
        ...
        $result2 = $db->prepare("select count(*) from actividad where archivo ='$arc' and cod_aten='$codaten' and user='".$user."'  and id_contacto<=98 and orden_servicio!='$a' and id_contacto!='' ");
        $result2->execute();
        $h = $result2->fetch();
        if($h[0]>0){
               ...
               $dados = array('mensaje' => "Usted no puede ingresar mas de 2 atenciones diarias ");
                   echo json_encode($dados);
                    ...
                    if(!$query){
                     ...
                     $result4 = $db->prepare("SELECT sum(puntos) FROM actividad where orden_servicio='".$a."'");
                     $result4->execute();
                     $fila = $result4->fetch();
                        $total=$fila['total'];
