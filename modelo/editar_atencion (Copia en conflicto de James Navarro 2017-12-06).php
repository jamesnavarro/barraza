...
$vencimiento = date('Y-m-d' , $nuevafecha);
	...
	$PULSO = $_POST["PULSO"];
        ...
        $sql = "UPDATE `actividad` SET `urgente`='No',`Description`='".$descripcion_t."',`estado`='".$estado_t."',`fecha_mod_ta`='".$fecha_mod_t."',`inf_adicional`='".$tratamiento."',`PA`='".$PA."',`PULSO`='".$PULSO."',`FR`='".$FR."',`Valoracion`='".$Valoracion."', `duracion`='".$time."', `registro`='".date("Y-m-d")."' , `puntos`='".$puntos."'  WHERE `Id`='".$_GET["cod"]."';";
 ...
 $sql10 = "SELECT * FROM actividad where `Id`='".$_GET["cod"]."' limit 1";
        ...
        $sql32 = "UPDATE `pacientes` SET `ultima_atencion`='".$fecha."', estado='Activo' WHERE  `id_paciente`='".$paciente."'";
         mysql_query($sql32);
