...
$cambio = valorEnLetras($_POST["total"]); 
...
return $rd; //Retornar el resultado 
      ...
      if($factura<3000){
             ...
             $sql = "UPDATE `facturas` SET `letras`='".$cambio."', `copagos`='".$cp."',`forma_pago`='".$forma_pago."',`meses`='".$meses."',`pago_pendiente`='".$pago_pendiente."',`total`='".$total."',`informacion`='".$info."',`fechai`='".$fi."',`fechaf`='".$ff."',`fecha_registro`='".$fr."' WHERE  `numero_factura`='".$factura."';";
     
      }
       mysql_query($sql);
       
            $consulta= "select * from actividad WHERE relacionado=".$factura." ";                     
...
if($_POST['cambio']==0){
    $c = '';
                        $sqlr = "UPDATE `actividad` SET `prioridad`='activa', relacionado='".$c."', factura_anulada='$factura' WHERE `orden_servicio`='".$oi."';";
                        mysql_query($sqlr);
                        
                        $insu = "UPDATE `insumos_asignados` SET `facturado`='".$c."', factura_anulada='$factura' WHERE  `rel_atencion`='".$oi."'";
                        mysql_query($insu);
                        $med = "UPDATE `medicamentos_asig` SET `facturado`='".$c."', factura_anulada='$factura' WHERE  `rel_atencion`='".$oi."'";
                        mysql_query($med);
                       
                        $lab = "UPDATE `laboratorio_asig` SET `facturado`='".$c."', factura_anulada='$factura' WHERE  `rel_atencion`='".$oi."'";
                        mysql_query($lab);
                        $pro = "UPDATE `productos_vendidos` SET `facturado`='".$c."', factura_anulada='$factura' WHERE  `rel_atencion`='".$oi."'";
                        mysql_query($pro);    
                        
                        $sqlf = "UPDATE `facturas` SET `numero_factura`='".$c."', factura_anulada='$factura' WHERE  `numero_factura`='".$factura."';";
                        mysql_query($sqlf);
                        }else{
     $c = $_POST['cambio'];
                        $sqlr = "UPDATE `actividad` SET `prioridad`='Facturado', relacionado='".$c."' WHERE `orden_servicio`='".$oi."';";
                        mysql_query($sqlr);
    ...
    echo "<script language='javascript' type='text/javascript'>";
      ...
      echo "<script language='javascript' type='text/javascript'>";
        echo "location.href='../vistas/?id=facturacion_finalizada&fact=".$_POST['cambio']."'";
        echo "</script>";  
}
     ...
     if($factura<3000){   
       $a2 = '<a href="../vistas/?id=facturacion_finalizada&fact='.$factura.'">Factura # '.$factura.'</a>';
        ...
        echo "<script language='javascript' type='text/javascript'>";
        echo "location.href='../vistas/?id=facturacion_recibo_atenciones&fact=".$factura."'";
        echo "</script>";
     }else{
         $a2 = '<a href="../vistas/?id=facturacion_finalizada&fact='.$factura.'">Factura # '.$factura.'</a>';
         $sqlr = "INSERT INTO `modificaciones` (`descripcion`,`modulo`, `por`) ";
        ...
        echo "<script language='javascript' type='text/javascript'>";
        echo "location.href='../vistas/?id=facturacion_finalizada&fact=".$factura."'";
        echo "</script>";
     }
    ?>