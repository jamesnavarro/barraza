...
if($factura<3000){
    $sql = "UPDATE `recibo_caja` SET `saldo`='".$saldo."', `letras`='".$cambio."', `copagos`='".$copagos."',`forma_pago`='".$forma_pago."',`meses`='".$meses."',`fechai`='".$fechai."',`fechaf`='".$fechaf."',`pago_pendiente`='".$pago_pendiente."',`total`='".$total."',`informacion`='".$info."' WHERE  `numero_recibo`='".$factura."';";
      
       mysql_query($sql);
       $status = "ok";
         $a2 = '<a href="../vistas/?id=facturacion_v&fact='.$factura.'">Factura # '.$factura.'</a>';
         $sqlr = "INSERT INTO `modificaciones` (`descripcion`,`modulo`, `por`) ";
            $sqlr.= "VALUES ('Se modifico la factura de venta', '".$a2."', '".$_SESSION['k_username']."')";
            mysql_query($sqlr, $conexion);
        echo "<script language='javascript' type='text/javascript'>";
        echo "location.href='../vistas/?id=facturacion_recibo_ventas&fact=".$factura."'";
        echo "</script>";
}else{
      $sql = "UPDATE `facturas` SET `letras`='".$cambio."', `copagos`='".$copagos."',`forma_pago`='".$forma_pago."',`meses`='".$meses."',`fechai`='".$fechai."',`fechaf`='".$fechaf."',`pago_pendiente`='".$pago_pendiente."',`total`='".$total."',`informacion`='".$info."' WHERE  `numero_factura`='".$factura."';";
       ...
       $status = "ok";
         ...
         $sqlr = "INSERT INTO `modificaciones` (`descripcion`,`modulo`, `por`) ";
            ...
            mysql_query($sqlr, $conexion);
        echo "<script language='javascript' type='text/javascript'>";
        echo "location.href='../vistas/?id=facturacion_v&fact=".$factura."'";
        echo "</script>";
