...
if($_POST['cambio']==0){
     $sqlf = "UPDATE `facturas` SET `numero_factura`='nada', factura_anulada='".$factura."' WHERE  `numero_factura`='".$factura."';";
     mysql_query($sqlf);
     
     mysql_query("update equipos_asig set facturado='' where facturado='".$factura."' ");
    
}else{
      $sql = "UPDATE `facturas` SET `letras`='".$cambio."', `copagos`='".$copagos."',`forma_pago`='".$forma_pago."',`meses`='".$meses."',`fechai`='".$fechai."',`fechaf`='".$fechaf."',`pago_pendiente`='".$pago_pendiente."',`total`='".$total."',`informacion`='".$info."',`fecha_registro`='".$fr."' WHERE  `numero_factura`='".$factura."';";
      
       mysql_query($sql) or die(mysql_error());
}   
        $a2 = '<a href="../vistas/?id=facturacion_2&fact='.$factura.'">Factura # '.$factura.'</a>';
         $sqlr = "INSERT INTO `modificaciones` (`descripcion`,`modulo`, `por`) ";
            $sqlr.= "VALUES ('Se modifico la factura dealquiler', '".$a2."', '".$_SESSION['k_username']."')";
          ...
          if($_POST['cambio']==0){  
               ...
               echo "<script language='javascript' type='text/javascript'>";
                echo "location.href='../vistas/?id=facturacion_2&fact=".$factura."'";
               echo "</script>";
          }
