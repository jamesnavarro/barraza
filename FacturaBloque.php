...
function Header()
{
 ...
 $strConsulta3 = "select * from facturas where numero_factura='".$_GET['imprimir']."' and estado='".$_GET['est']."' ";
	...
	$this->SetFont('Arial','B',8);
            ...
            $this->Cell(0,10,'Factura  No. '.$pre.$fila3['numero_factura'],10,0,'R');
...
function Footer()
{
	...
	$pdf->SetWidths(array(10, 17, 90, 10, 15, 15, 15));
    ...
    $pdf->SetTextColor(0);
		...
		for($i=0;$i<1;$i++)
			{
				$pdf->Row(array('ITEM', 'COD.', 'DESCRIPCION', 'CANT.', 'P. X UND','COPAGO', 'TOTAL'));
			}
	
	$historial = $con->conectar();
        
        $consultaoe= "select *,count(cant_ins), sum(cuota_pagada/cant) as cuota from actividad where relacionado='".$_GET['imprimir']."' and estado='Completada' and factura='".$_GET['est']."' GROUP BY cod_aten,precio_total ";
...
while($fila=  mysql_fetch_array($resultoe)){
    ...
    $copago += $fila['cuota'];
    $contador++;
    $pdf->Row(array($contador, $fila['cod_aten'], substr(utf8_decode($fila['Description']), 0, 65), $fila['count(cant_ins)'], number_format($fila['precio_total']), number_format(($fila['cuota'])),number_format(($fila['count(cant_ins)']*$fila['precio_total'])-$fila['cuota'])));
