...
$queEmp = "
    select a.cod_insumo, a.cantidad, b.nombre_insumo from insumos_asignados a, insumos b where a.cod_insumo=b.codigo and a.numero_orden='".$_GET['imprimir']."'
    union 
    select a.cod_lab, a.cantidad, b.nombre_lab from laboratorio_asig a, laboratorio b where a.cod_lab=b.cod_lab and a.numero_orden_lab='".$_GET['imprimir']."'
    union
    select b.codigo, a.cantidad, b.nombre_medicamento from medicamentos_asig a, medicamentos b where b.codigo_int=a.cod_med and a.numero_orden='".$_GET['imprimir']."'
    union 
    select b.codigo_interno, a.cantidad , b.nombre from productos_vendidos a, productos b where b.codigo_interno=a.cod_pro and a.numero_orden_v='".$_GET['imprimir']."'
    union
    select  a.cod_aten, a.cant, a.Description from actividad a where a.orden_servicio='".$_GET['imprimir']."'";
...
/////////////////////////////////////
$consulta= "select a.*, b.*, c.* from ordenes a, pacientes b, sis_empresa c where a.id='".$_GET['imprimir']."' and a.id_paciente=b.id_paciente and b.id_empresa=c.rips";
$result=  mysql_query($consulta);
while($fila=  mysql_fetch_array($result)){
...
$txttit = "<b>                                                                      MINISTERIO DE LA PROTECCION SOCIAL\n";
...
$pdf->ezText("INFORMACION DEL PRESTADOR (solicitante)", 10);
$pdf->ezText("___________________________________________________________________________________________", 10);
