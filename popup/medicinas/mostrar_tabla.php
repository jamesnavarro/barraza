...
if(isset($_GET['nombre'])){
                    ...
                    $colx = 'where nombre_medicamento like "%'.$_GET['nombre'].'%" ';
            }else{
                    $colx='';
            }
if(isset($_GET['page'])){
...
$request=mysql_query('SELECT count(*) FROM medicamentos '.$colx.' ');
            if($request){
                ...
                ?>
                            <table class="table table-bordered table-condensed table-hover">
                                ...
                                <tbody>
                                    <?php
                                    if(isset($_GET['nombre'])){
                                        $col = 'where concat(nombre_medicamento) like "%'.$_GET['nombre'].'%" ';
                                    }else{
                                        $col = '';
                                    }
                                    $sql = mysql_query("SELECT * FROM medicamentos  $col ".$limit);
			$item = 0;
			if(mysql_num_rows($sql)>0){
				while($mostrar = mysql_fetch_array($sql)){
					...
					echo '<tr>
<td>'.$item.'</td>
<td><a href="../popup/medicinas.php?codigo='.$mostrar['codigo_int'].'">'.$mostrar['codigo_int'].'</a></td>
<td>'.$mostrar['nombre_medicamento'].'</td>'; }
			}else{
