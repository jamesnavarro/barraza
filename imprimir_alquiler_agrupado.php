...
function Header()
{
 ...
 $strConsulta3 = "select * from facturas where numero_factura='".$_GET['imprimir']."' and estado='".$_GET['estado']."' ";
	...
	$this->SetFont('Arial','B',8);
        $this->Cell(155,10,'Factura de Venta No. '.$pre.$fila3['numero_factura'],0,0,'R');
	$this->Ln(0);
}
function Footer()
{
	...
	$strConsulta = "select *, sum(cantidad) as can from equipos_asig a, alquiler b where a.cod_equipo=b.codigo and a.facturado='".$_GET['imprimir']."' group by cod_equipo";
	...
	for ($i=0; $i<$numfilas; $i++)
		{
			...
			$pdf->SetFillColor(255,255,254);
    			$pdf->SetTextColor(0);
			$pdf->Row(array($filad['numero_orden_a'], $filad['autorizacion'], $filad['cod_equipo'], $filad['nombre'], $filad['anexo'], $filad['can'], number_format($filad['precio_a']), number_format($filad['precio_a']*$filad['can']*$filad['meses'])));
			
                        $to += $filad['precio_a']*$filad['can']*$filad['meses'];
		}
