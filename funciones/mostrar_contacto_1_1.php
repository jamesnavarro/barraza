...
$request=Connection::runQuery('SELECT a.*, b.*, c.* FROM actividad a, pacientes b, sis_empresa c WHERE b.id_empresa=c.rips and a.user="'.$user_u.'" and a.id_paciente=b.id_paciente group by a.orden_servicio');
if($request){
//    echo'<hr>';
	...
	{       
                 ...
                 if($modulo_rCL=='Clientes' && $ver_rCL=='Habilitado'){$ver='<a href="../vistas/contacto_potencial.php?codigo='.$row["id_paciente"].'">';}else{$ver='';}
           if($modulo_rCL=='Clientes' && $editar_rCL=='Habilitado'){$b='<a href="../form_editar/formulario_editar_contacto_potencial.php?codigo='.$row["id_paciente"].'"><img src="../imagenes/modificar.png" alt="ver" height="15px" width="15px"></a>';}else{$b='';}
          if($row["id_contacto"]!=''){$por='<td>'.$row["id_contacto"].' %<font></a></td>';}else{$por='<td>0 %</td>';}
           ...
           $table = $table.'<tr><td>'.$ver2.''.$row["orden_servicio"].'</font></td><td>'.$ver.''.$row["nombres"].' '.$row["apellidos"].'<font></a></td>
              <td>'.$color.''.$row["StartTime"].'</font></td> <td>'.$color.''.$row["EndTime"].'</font></td><td>'.$row["Description"].'</font></td>
                    <td>'.$b.'</td><td><font color="blue">'.$row["positivo"].' %</font></td><td><font color="orange">'.$row["negativo"].' %</font></td>
                        <td>'.$tp.' %</td>
</tr>';
