...
$requestA=Connection::runQuery('(select * from actividad where tarea="Actividad" and relacionado="Cuenta" and id_seleccionado='.$id_empresa.')');
$requestB=Connection::runQuery('(select * from actividad where tarea="Reunion" and relacionado="Cuenta" and id_seleccionado='.$id_empresa.')');
$requestC=Connection::runQuery('(select * from actividad where tarea="Llamada" and relacionado="Cuenta" and id_seleccionado='.$id_empresa.')');
$requestD=Connection::runQuery('(select * from sis_notas where seleccion_n="Cuenta" and id_seleccion_n='.$id_empresa.')');
...
$table = $table.'</thead>';
                    ...
                    while($row=mysql_fetch_array($requestD))
	{
                    ...
                    </td><td><a href="../modelo/eliminar.php?elimi2='.$row["id_nota"].'"><img src="../imagenes/eliminar.png" alt="ver" height="20px" width="20px"></a>
                    </td></tr>';}
                    }else{
                        if($row['estado_n']=="Completada" || $row['estado_n']=="Aplazada" || $row['estado_n']=="Realizada" || $row['estado_n']=="No Realizada" || $row['estado_n']=="Nota"){
                    $table = $table.'<tr><td><a href="../vistas/mostrar_toda_act.php?codigo='.$row["id_nota"].'">'.$row["asunto_n"].'</a></td><td>'.$row['estado_n'].
