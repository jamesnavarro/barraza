...
$request=mysql_query("select *, sum(a.porcentaje) as total from actividad a, ordenes b where b.id=a.archivo and a.archivo='".$_GET['cod']."' group by a.orden_servicio order by a.orden_servicio");
     }else{
      $request=mysql_query("select *, sum(a.porcentaje) as total from actividad a, ordenes b where a.user='".$_SESSION['k_username']."' and b.id=a.archivo and b.estado_ord='En proceso' and a.estado='No iniciada' and a.archivo='".$_GET['cod']."' group by a.orden_servicio order by a.orden_servicio");
   ...
   $table = '<table class="table table-bordered table-striped table-hover" id="">';
	...
	{       
          ...
          if($row["urgente"]=='Si'){
               ...
               $var= 100 - $row['total'];
            $table = $table.'<tr><td>'.$ur.'<td>'.$ver.''.$color.''.$row['Description'].' </font><br>Estado: <b>'.$rev.'</b><br>Fecha Expedida:'.$row['fecha_exp'].'<br>Fecha Vencimiento:'.$row['fecha_ven'].' </a>'.$fac.'</td><td>'.$color.''.$row['orden_servicio'].'</font></td><td>'.$ver.''.$color.''.$row['orden_externa'].'</font></td><td>'.$ver.''.$color.''.$row["cant_ins"].'<font></a></td></td>
