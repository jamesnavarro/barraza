...
<header><form action="../modelo/insertar_equipo_asig.php?orden=<?php echo $orden_interna ?>&fi=<?php echo $fi ?>&ff=<?php echo $ff ?>&pac=<?php echo $idp ?>&descrip=<?php echo $descr_alq ?>" method="post" name="fcontacto">
                                <select name="insumo2" id="select2_1" style="width:25%;" required>	
                                                          ...
                                                          <?php
                                                            ...
                                                            $consulta= 'SELECT a.codigo,a.nombre  FROM alquiler a, precios_alquiler b, sis_empresa c WHERE b.id_empresa=c.id_empresa and c.rips="'.$id_rips.'" and a.id=b.id_alquiler';                     
        ...
        Precio: <span id="p"><input type="text" style="width:10%;" name="precio_alq" id="precio_alq" ></span>
                                 ...
                                 # Archivo :<input name="oi" type="text" readonly style="width:40px;" value="<?php echo $max; ?>"> Autorizacion<select name="autorizacion" style="width:130px;">
                                                            ...
                                                            $consulta= "SELECT orden FROM `ordenes` WHERE id='".$orden_interna."'";                     
                                                            $result=  mysql_query($consulta);
                                                            while($fila=  mysql_fetch_array($result)){
                                                            $valor1=$fila['orden'];
                                                            $valor2=$fila['orden'];
...
<?php }
    $_SESSION['o']= $idp;           
$request=mysql_query("SELECT * FROM equipos_asig a, alquiler b, ordenes c WHERE c.id=a.numero_orden_a and a.cod_equipo=b.codigo and a.numero_orden_a='".$orden_interna."'  group by a.id_equipo_a");
