...
function Header()
{
 ...
 $strConsulta3 = "select * from facturas where numero_factura='".$_GET['imprimir']."' and estado='".$_GET['estado']."' ";
	...
	$this->SetFont('Arial','B',8);
        $this->Cell(155,10,'Relacion Factura No. '.$pre.$fila3['numero_factura'],0,0,'R');
	$this->Ln(0);
}
function Footer()
{
	...
	// N�mero de p�gina
        ...
        //$this->Cell(0,10,'Nota: Tambien pueden consignar a la CUENTA CORRIENTE DE BANCOLOMBIA No. 44200000554 ',0,0,'C');
	...
	$pdf->SetWidths(array(20, 20, 40, 60, 10, 15, 15));
	$pdf->SetFont('Arial','B',6);
    ...
    $pdf->SetTextColor(0);
		...
		for($i=0;$i<1;$i++)
			{
				$pdf->Row(array( 'AUTORIZACION','NUMERO', 'PACIENTE', 'DESCRIPCION', 'CANT.', 'P X UND', 'TOTAL'));
			}
	...
	for ($i=0; $i<$numfilas; $i++)
		{
			...
			$filad = mysql_fetch_array($historial3);
                        ...
                        $pacientesr = mysql_query("SELECT * FROM pacientes where id_paciente='".$filad['id_paciente']."' ");
                        $pa = mysql_fetch_array($pacientesr);
			$pdf->SetFont('Arial','',0);
			...
			if($i%2 == 1)
			{
    			...
    			$pdf->SetTextColor(0);
				$pdf->Row(array( $filad['autorizacion'],$pa['documento'].':'.$pa['numero_doc'], $pa['nombres'].' '.$pa['apellidos'], $filad['nombre'], $filad['cantidad'], number_format($filad['precio_a']), number_format($filad['precio_a']*$filad['cantidad']*$filad['meses'])));
			}
			else
			{
		        $pdf->SetFillColor(255,255,254);
    			$pdf->SetTextColor(0);
                        ...
                        $to += $filad['precio_a']*$filad['cantidad']*$filad['meses'];
		}
$pdf->Cell(0,3,'Nota :  '.$fila3['informacion'],0,1);
$pdf->Ln(0);
$pdf->Cell(0,3,'_______________________________________________________________________________________________________________________________________________',0,1,'C');
      $cambio = valorEnLetras($to-$fila3['copagos']); 
        $pdf->SetFont('Arial','',8);
        $pdf->Cell(0,3,'Copagos :  '.number_format($fila3['copagos']).'                  Servicios : '.number_format($to).'                Materiales y Medicamentos :'.number_format(0),0,1);
        $pdf->Ln(2);
        $pdf->Cell(0,3,'Son :  '.$cambio,0,1);
        $pdf->SetFont('Arial','B',12);
        $pdf->Cell(0,3,'Total Neto a Pagar $ '.number_format($to-$fila3['copagos']),0,1,'R');
        mysql_query("update facturas set total='".($to-$fila3['copagos'])."', letras='$cambio' where numero_factura='".$_GET['imprimir']."' ");
$pdf->Output();
